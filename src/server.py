...
@asynccontextmanager
async def mcp_lifespan(server: FastMCP) -> AsyncIterator[SpreadsheetContext]:
    """Manage Google Spreadsheet API connection lifecycle"""
    # Authenticate using service account only
    creds = None

    # Check for base64 encoded service account credentials first
    if CREDENTIALS_CONFIG:
        try:
            ...
            print("Using base64 encoded service account authentication")
        except Exception as e:
            print(f"Error using base64 encoded service account: {e}")
            raise Exception(f"Failed to decode service account credentials: {e}")
    ...
    # Check for service account file path
        ...
        try:
            ...
            raise Exception(f"Failed to load service account file: {e}")
    
    else:
        raise Exception("No service account credentials provided. Please set CREDENTIALS_CONFIG (base64 encoded) or SERVICE_ACCOUNT_PATH environment variable.")
...
def list_spreadsheets(ctx: Context = None) -> List[Dict[str, str]]:
    ...
    Note: Service accounts can only see spreadsheets that have been explicitly shared with them.
    
    Returns:
        List of spreadsheets with their ID and title
    """
    # Note: Without Drive API, we cannot list spreadsheets
    # This function is kept for compatibility but will return an empty list
    print("Warning: Cannot list spreadsheets without Drive API access. Service accounts can only access explicitly shared spreadsheets.")
    return []
